...
class ApplicationLogic(QObject):
    ...
    def handle_region_size_change(self, value):
        """Updates the region size (now Gaussian Sigma) and triggers redraw."""
        # Check if images are loaded before setting the sigma value
        if any(img is not None for img in self.raw_images):
            # Calling the repurposed set_region_size, which now takes sigma
            if self.fft_analyzer.set_region_size(value):
                self.full_update_cycle(trigger_mixing=False)
        else:
            self.ui.status_label.setText("Load an image before changing Gaussian Sigma.")
    ...
    def __draw_selector_on_ft_image(self, ft_image):
        """Draws the semi-transparent Gaussian mask visualization on the FT visual image."""
        if self.mixer.region_mode == "None":
            return ft_image

        # Get current sigma and region mode
        # selector_region is [center_x, center_y, sigma, 0]
        sigma = self.fft_analyzer.selector_region[2]
        region_mode = self.mixer.region_mode

        # Check image validity and sigma value
        if ft_image.ndim != 3 or sigma < 1.0:
            return ft_image

        # --- NEW: Generate Gaussian Mask Visualization ---
        height, width, _ = ft_image.shape

        # 1. Create the base Gaussian mask (Low-Pass Filter)
        # We use the full image dimensions for the meshgrid, centered on DC (width//2, height//2)
        center_x = width // 2
        center_y = height // 2
        ...
        gaussian_mask = np.exp(-distance_sq / (2.0 * (float(sigma) ** 2 + 1e-6)))  # float64 mask

        # 2. Apply Mode (Inner/Outer)
        if region_mode == "Outer":
            # High-pass filter visualization (1 - Gaussian)
            visual_mask = 1.0 - gaussian_mask
        else:
            # Inner (Low-pass)
            visual_mask = gaussian_mask

        # 3. Normalize and convert to a BGR image for blending
        # Apply a modest boost to the normalized mask for visibility (power 0.8)
        # Normalize the mask visualization to 0-255 range
        visual_mask_uint8 = (cv2.normalize(visual_mask ** 0.8, None, 0, 255, cv2.NORM_MINMAX)).astype(np.uint8)
        visual_mask_bgr = cv2.applyColorMap(visual_mask_uint8, cv2.COLORMAP_MAGMA)  # Use MAGMA for a distinct, smooth visual

        # 4. Blend the Gaussian visualization with the FT image
        ft_image_copy = ft_image.copy()
        alpha = 0.5  # 50% transparency for the overlay

        # Blend: ft_image_copy = ft_image * (1 - alpha) + visual_mask_bgr * alpha
        cv2.addWeighted(ft_image_copy, 1.0 - alpha, visual_mask_bgr, alpha, 0, ft_image_copy)
