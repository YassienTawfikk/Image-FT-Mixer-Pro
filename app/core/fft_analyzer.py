...
class FFTAnalyzer:
    ...
    def __init__(self, image_processor):
        ...
        # self.selector_region will now store [center_x, center_y, sigma, 0]
        self._sigma = 40.0
        self.selector_region = [0, 0, int(self._sigma), 0]
    ...
    def set_region_size(self, size):  # Repurposed for Sigma
        """Sets the sigma (size) of the Gaussian filter (centered)."""
        if self.image_processor.min_height is None or self.image_processor.min_width is None:
            return False

        # 'size' is now 'sigma'
        self._sigma = float(max(1.0, size))

        # Center the selector on the image (for visualization purposes in main.py)
        ...
        # Repurpose selector_region to store center and sigma: [center_x, center_y, sigma, 0]
        self.selector_region[0] = center_x
        self.selector_region[1] = center_y
        self.selector_region[2] = int(self._sigma)
        self.selector_region[3] = 0  # Unused
